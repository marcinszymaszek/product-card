    ...
    <form id="app" @submit="checkForm">
      ...
      <div class="size-buttons">
        <label
          ...
          v-for="size in sizes"
          @click="changeSizePrice(size.priceGross)"
          :key="size.value"
        >
...
export default {
  ...
  mounted: function() {
    const result = async () => {
      ...
      const sizesArray = data.configurableAttributes.size;
      const pricesArray = data.configurablePrices[0].prices;

      this.basicPrice = data.basePriceGross;
      this.colorDefault = data.configurableAttributes.color[0].value;
      sizesArray.map((size, i) => {
        return (this.sizes[i].value = size.value);
      });
      pricesArray.map((price, i) => {
        return (this.sizes[i].priceGross = price.priceGross);
      });
    };
    result();
  },
  ...
  methods: {
    changeSizePrice(price) {
      ...
      this.$emit("selectedSizePrice", this.basicPrice);
      // }
      // if (this.sizes.sizeId === prices.attributeId) {
      //   let i;
      //   this.price = prices[i].priceGross;
