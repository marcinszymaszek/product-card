...
export default {
  ...
  mounted: function() {
    const result = async () => {
      ...
      sizesArray.map((size, i) => {
        return (
          (this.sizes[i].sizeName = size.value),
          (this.sizes[i].sizeId = size.id)
        );
      });
    };
    result();
  ...
  methods: {
    changeSizePrice: function(event) {
      const result = async () => {
        const response = await fetch("../../static/product.json");
        const data = await response.json();
        const prices = data.configurablePrices[0].prices;
